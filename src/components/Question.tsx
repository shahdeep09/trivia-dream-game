...
const Question = ({
  ...
  // Always use ScrollArea for consistent behavior across all screen sizes
  return (
    <ScrollArea className="w-full max-w-2xl mx-auto h-[calc(100vh-150px)]">
      <div className="px-4">
        {/* Crorepati Logo */}
        ...
        <div className="bg-millionaire-primary p-6 rounded-lg border border-millionaire-highlight mb-6 shadow-lg animate-fade-in hexagon">
          <h2 className="text-3xl md:text-5xl text-center text-millionaire-light font-medium mb-2">
            {question.text}
          </h2>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
          {question.options.map((option, index) => (
            ...
            >
              ...
              <span className="flex-1 text-left text-2xl">{option}</span>
            </button>
          ))}
        </div>
        ...
        {selectedIndex !== null && !revealAnswer && (
          <div className="my-6 flex justify-center">
            <button
              className="bg-millionaire-gold hover:bg-yellow-500 text-millionaire-primary font-bold py-2 px-6 rounded-full animate-pulse text-xl"
